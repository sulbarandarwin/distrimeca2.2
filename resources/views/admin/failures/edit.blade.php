<x-app-layout>
    @section('title', __('Editar Registro de Falla'))
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Editar Registro de Falla para:') }} {{ $productFailure->product->name ?? 'Producto Desconocido' }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.failures.update', $productFailure) }}">
                    @csrf
                    @method('PUT')
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <select name="product_id" id="product_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-700">
                                ...
                                @foreach($products as $id => $name)
                                    <option value="{{ $id }}" {{ old('product_id', $productFailure->product_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input type="datetime-local" name="failure_date" id="failure_date" value="{{ old('failure_date', $productFailure->failure_date->format('Y-m-d\TH:i')) }}" required class="mt-1 block w-full" />
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-700">{{ old('description', $productFailure->description) }}</textarea>
